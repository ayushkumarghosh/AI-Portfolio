...
function TabActivationHandler({ setActiveTab }: { setActiveTab: (tab: string) => void }) {
  ...
  useEffect(() => {
    ...
    if (isMounted && highlightParam && highlightParam.includes('sap-flutter')) {
      console.log('Flutter experience detected in URL, activating Flutter tab directly')
      setActiveTab('flutter')
    }
  }, [highlightParam, setActiveTab, isMounted])
  ...
  useEffect(() => {
    ...
    if (highlightParam) {
      ...
      // Add a larger delay to ensure DOM is fully loaded
      ...
      }, initialDelay) // Larger delay to ensure DOM is ready
    ...
    <>
      ...
      <div className={`page-transition-wrapper ${isLoaded ? 'loaded' : ''}`}>
        <div className="container mx-auto py-12 px-4 md:px-6">
          <h1 className="text-4xl font-bold mb-8 text-primary">Experience</h1>
          <Card id="sap" className="mb-8 card-hover border border-secondary/20">
          ...
          <Card id="ziroh" className="card-hover border border-secondary/20">
            <CardContent className="p-6">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
                <div className="flex items-start gap-3">
                  <div className="bg-primary/10 p-2 rounded-full mt-1">
